...
wedoContent.addEventListener((e)=>{
  if(e.target.classList.contains("wedo-item") && !e.target.classList.contains("wedo-is-active")){
    document.querySelectorAll('.wedo-item').forEach((item)=>{
      item.classList.remove(".wedo-is-active");
    })
    e.target.closest('.wedo-item').classList.add('.wedo-is-active');
  }
})
