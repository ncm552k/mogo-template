...
wedoContent.addEventListener('click',(e)=>{
  ...
  if (e.target.closest('.wedo-item-header').classList.contains("wedo-item-header") && !e.target.closest('.wedo-item').classList.contains('wedo-is-active')){
    ...
    document.querySelectorAll(".wedo-item").forEach((item)=>{
      ...
      item.querySelector('.wedo-item-arrow').classList.remove('fa-angle-up');
      item.querySelector('.wedo-item-arrow').classList.add('fa-angle-down');
    })
